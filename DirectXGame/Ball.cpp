...
//				//頂点インデックス一個分の文字列鵜をストリームに変換して解析しやすくする
...
ID3D12Device* Ball::device = nullptr;
ID3D12GraphicsCommandList* Ball::cmdList = nullptr;
Ball::PipelineSet Ball::pipelineSet;
Camera* Ball::camera = nullptr;

void Ball::StaticInitialize(ID3D12Device * device, Camera* camera)
{
	// 再初期化チェック
	assert(!Ball::device);
...
void Ball::CreateGraphicsPipeline()
{
	HRESULT result = S_FALSE;
	ComPtr<ID3DBlob> vsBlob; // 頂点シェーダオブジェクト
	ComPtr<ID3DBlob> psBlob;    // ピクセルシェーダオブジェクト
